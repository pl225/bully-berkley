...
public abstract class Processo {
	...
	int intervaloAtualizacao; // intervalo aleatorio de atualizacao do relogio
	...
	public Processo (int pid) {
		this.pid = pid;
		this.setAtualizandoRelogio(false);
		this.relogioLocal = 0;
	...
	public synchronized void iniciarAtualizacaoRelogio () {
		this.setAtualizandoRelogio(true);
		new Thread(new AtualizacaoRelogio(this)).start();
	}
	...
	public synchronized void executarAlgoritmo(Mensagem mensagem) {
		if (mensagem.getTipoMensagem() == TipoMensagem.ELEICAO) { // verificar mais seguranca para nao executar bully mais de uma vez
        	...
        	System.out.println("Eu, o processo " + this.pid + " não estou mais na disputa");
        	this.setEstouNaDisputaBully(false);
	...
	private void iniciarEsperaBully() {
		if (this.aguardandoResultadoBully == null) {
			...
			this.aguardandoResultadoBully = new Thread(new InicializacaoAlgoritmoBully(this));	
	...
	protected void iniciarSincronizacaoBerkley() {
		...
		new Thread(new InicializacaoAlgoritmoBerkley(this)).start();
	...
	public boolean isAtualizandoRelogio() {
		return isAtualizandoRelogio;
	}
	
	public void setAtualizandoRelogio(boolean isAtualizandoRelogio) {
		this.isAtualizandoRelogio = isAtualizandoRelogio;
	}
	
	void incrementarRelogio (long valor) {
		this.relogioLocal += valor;
	}
}
